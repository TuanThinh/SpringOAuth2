...
public class MongoTokenStore implements TokenStore{
    ...
    public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String username) {
        return findTokensByCriteria(
                Criteria.where(OAuthAccessToken.CLIENT_ID).is(clientId)
                        .and(OAuthAccessToken.USERNAME).is(username));
    }

    @Override
    public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
        return findTokensByCriteria(Criteria.where(OAuthAccessToken.CLIENT_ID).is(clientId));
    }
